...
# As with the find_svos method, help with this came from the following project
...
# Probably missing some characters and specifics!
...
# A helper method - repeats the task of adding a token to a dependency list.
...
def add_token_and_dependencies(token, dependency_list):
    subtree = list(token.subtree)  # Check all sub-parts of a token
...
# -> Determing the verb's descriptive content, such as adverbs
...
def find_svos(tokens):
    ...
    for verb in verbs:
        # Determine subject as any part of left that is a nsubj/nsubjpass/etc., which for spaCy is pretty accurate
        try:
        ...
        # Get object parts after the verb
        object_words = []
        for token in verb.rights:
            if token.pos_ != 'VERB':  # Don't re-add verbs - stops cutting off later SVOs
    ...
    for sentence in doc.sents:
        ...
        for svo in sentence_svos:
            ...
            for obj_word in svo_object_words:
                if obj_word not in all_tokens:  # Run through the verb
